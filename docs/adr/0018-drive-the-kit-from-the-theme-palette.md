...
None of that is Tanto's source of truth. Tanto's theme palette is, so
...
Type was the part that was unowned. #10 retired Tanto's own `Typography`, which
...
`Style.resolvedFontFamily` get that concrete name. Tanto never asks Qt for a
...
theme without Tanto naming any of them.
...
What the seam does not drive stays the kit's. `Color.urgent` is one: Tanto's
...
Tanto disagree on appearance the kit wins (0017), and these are the kit's own
appearance rather than Tanto's palette.

Letting the kit read the desktop's colours instead was the other candidate, and
it is rejected. `themePath()` already prefers
`~/.local/state/omarchy/current/theme/tanto.json` on Linux, and
`integrations/omarchy/tanto.json.tpl` already renders the desktop's own font
family into it, so a user's Omarchy theme reaches Tanto's chrome through the
palette Tanto owns — including the parts, like the Private-window separation and
