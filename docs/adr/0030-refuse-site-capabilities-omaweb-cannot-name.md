...
Site capabilities are decided from an allow-list of named policies rather than from whatever the
engine happens to ask about. A capability Omaweb has no policy for is refused without a question
being asked: a question whose only honest answer is no is not a question, and an engine that grows a
new capability must not be able to reach the reader through Omaweb before Omaweb has decided what
the answer means.
...
earlier answer could have covered; offering to remember one would be the reader agreeing to a use
they will never be shown again. Adapters translate the engine's own capability identifiers into
these names, so the policy is written in Omaweb's words and an untranslated identifier is refused
...
be given one origin's allowance inside one Space: it lives in memory beside the Site permissions a
...
it arrives on the command line or through the environment's engine flags, because nothing the reader
can see on a page would tell them the boundary had been given away.

Nothing an engine decides for itself may put a site's decisions beyond the reach of the reset that
is offered for them. QtWebEngine stores granted permissions in a profile of its own by default and
answers a later request from that store without asking the embedder, which would make Omaweb's own
reset appear to work and change nothing. Engines are therefore told to ask every time, so Omaweb's
store is the only place a site's decisions live, and an origin's reset drops the engine's own record
of it as well. What remains outside reach is a capability a loaded page is already holding: engines
answer that from a store keyed by the frame that asked, and reloading reuses the frame, so Site
information says that opening the site again is what takes it back rather than offering a reload
that would not.
...
script. Neither action is described as the other, and a size is only ever counted against the action
that can move it.

Site information states; it does not ask. Every question it leads to is asked in the centred dialog
every other question about the browser is asked in, and the panel closes behind it so one surface
holds the question. A confirmation squeezed in beside the thing it is about has no room to name the
scope that is going, and the three clearings here are three different sizes that only their wording
tells apart. The same applies to allowing a third party: the reader is choosing among embedded
origins they cannot judge by name, so the choice is a row that names the whole origin and the flow
it would be for, under a sentence saying to allow one only when a sign-in or payment is failing. The
panel lists what was refused and what is allowed, and stops there.

Where the engine cannot answer for part of this contract, Site information says so rather than
drawing a reassuring blank. Reporting a certificate failure at all and refusing a third party are
named capabilities an adapter may lack. How much site data a Space holds is not a capability: it is
measured on disk at the path Omaweb assigned the engine's profile, and is shown only where the
adapter claims to keep a persistent profile.
