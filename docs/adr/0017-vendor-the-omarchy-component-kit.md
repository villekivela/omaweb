...
Omaweb's shared controls come from the Omarchy shell's QML kit rather than from a
second set of Omaweb-owned equivalents. Both projects are QML on Qt 6, the kit
already covers what browser chrome needs — buttons, text inputs, toggles,
dropdowns, sliders, confirm dialogs, border and panel primitives — and Omaweb
...
keeping Omaweb's property names and the accessibility annotations the kit does not
...
components look like themselves. The setting is process-wide, so Omaweb's own Qt Quick
...
theme on disk, so Omaweb's adapters pass colour per instance from the palette their
call sites already receive. Whether Omaweb later drives those singletons from
...
Type is not drilled the same way. Omaweb had its own `Typography` object resolving a
family and a size scale from the palette; the kit's `Style.font` is the same idea with
a fuller scale, so `Typography` is gone and every Omaweb surface reads `Style.font`
directly. Until `ThemeController` drives `Style`, that means the type scale follows the
kit's defaults rather than Omaweb's palette — the visible cost of having one scale
instead of two, paid deliberately. 0018 stops paying it.

Where the kit and Omaweb disagree on appearance, the kit wins: emphasis is a tinted,
...
Omaweb keeps its own control only where the kit has no equivalent at all — `SettingRow`,
