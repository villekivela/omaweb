...
stand-in view draws a sample page in the page palette where it would otherwise say no engine is
running. `--spaces` seeds a Work Space with pages of its own beside Personal, before the interface
loads, so a Space switch has somewhere to go. `--sample-lists` writes EasyList and EasyPrivacy into
the lab's content-blocking settings as current, so Content Blocking shows the lists a first run has
without fetching them. Pass `--capture <path>` to render one frame to a PNG and exit, which works
