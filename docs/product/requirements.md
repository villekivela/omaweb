...
- The vertical sidebar is an outline of the active Space, read top to bottom: the Space heading, the other Spaces, the current-address trigger, pinned tabs as icon-only buttons in full-width rows with capacity for three to five tabs, and ordinary tabs as single-line rows. Incomplete pinned rows divide their full width among the tabs they contain. Settings closes the sidebar; new tabs remain available through the Omnibar and keyboard commands rather than a sidebar button.
- Tabs can show site favicons or a two-character host code. The reader can turn favicons off and can choose whether favicon artwork is recoloured to the host-derived tint. Pinned-tab icons are larger than ordinary-tab icons.
