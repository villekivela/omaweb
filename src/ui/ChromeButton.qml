...
// takes an icon token and a word `Style.font.body`, where Omaweb's own button
...
// `accent`, so Omaweb's `hoverBackground` is gone: chrome is borderless and
...
Omarchy.Button {
    ...
    // A disabled control is not a keyboard stop, which is what Omaweb's own
