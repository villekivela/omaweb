...
QtObject {
    ...
    function releaseVisibleSpace() {
        for (const tabId in root.tabs) {
            if (root.tabs[tabId] === root.visibleSpaceId) delete root.tabs[tabId]
        }
    }
    ...
    function reconcile() {
        ...
        for (let index = 0; index < wanted.length; ++index) {
            ...
            if (root.host.engines[kept.tabId]) continue
            if (root.host.blankAddress(kept.url)) continue
            const profile = root.spaceProfiles.hostFor(kept.spaceId)
            if (!profile) continue
            const engine = root.host.createEngine(kept.tabId, kept.url, kept.spaceId,
                root.browser.profilePathForSpace(kept.spaceId), profile.profile)
            if (!engine) continue
        ...
        for (const tabId in root.tabs) {
            if (root.tabs[tabId] === root.visibleSpaceId) continue
    ...
    function report() {
        ...
        for (let index = 0; index < wanted.length; ++index) {
            ...
            report.push({
                ...
                "residentBytes": engine
                    ? ProcessResources.residentBytes(engine.renderProcessPid) : 0
            })
        }
        return report
    }
