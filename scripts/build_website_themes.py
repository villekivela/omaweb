#!/usr/bin/env python3
"""Render the website's per-theme assets: palettes, wallpapers, shots.
...
def favicon(palette: dict) -> str:
    ...
    shipping the icon's own palette as the theme's. The page's script does
    the same swap at runtime, by position rather than by literal: the ground
    is the first fill in the file and the mark the second, and a redesign
    that reorders them has to change both.
    """
...
def stamp(svg: str) -> str:
    ...
    A comment ahead of `<?xml ...?>` is not well-formed.
    """
    end = svg.find("?>")
...
def main() -> int:
    ...
    if default in palettes:
        (WEBSITE / "favicon.svg").write_text(favicon(palettes[default][0]), encoding="utf-8")
