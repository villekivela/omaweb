...
class Fallback(unittest.TestCase):
    ...
    def test_an_unreachable_api_is_retried_and_then_given_up_on(self):
        # In process rather than through the script, so the backoff between
        # attempts can be taken out of the run.
        delay = rewrite.RETRY_DELAY
        rewrite.RETRY_DELAY = 0
        try:
