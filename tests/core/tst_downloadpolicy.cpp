...
void DownloadPolicyTest::namesTheKindOfFileTheReaderIsBeingHanded_data()
{
    ...
    QTest::newRow("Windows program") << QStringLiteral("setup.exe") << QStringLiteral("executable");
    QTest::newRow("Java program") << QStringLiteral("tool.jar") << QStringLiteral("executable");
    QTest::newRow("shell script") << QStringLiteral("install.sh") << QStringLiteral("script");
    QTest::newRow("PowerShell script")
        << QStringLiteral("bootstrap.ps1") << QStringLiteral("script");
    QTest::newRow("desktop entry") << QStringLiteral("Invoice.desktop") << QStringLiteral("script");
    QTest::newRow("Debian package")
        << QStringLiteral("omaweb_1.0_amd64.deb") << QStringLiteral("installer");
    ...
    QTest::newRow("Apple disk image")
        << QStringLiteral("Omaweb.dmg") << QStringLiteral("disk image");
    QTest::newRow("optical image") << QStringLiteral("arch.iso") << QStringLiteral("disk image");
    QTest::newRow("zip archive") << QStringLiteral("photos.zip") << QStringLiteral("archive");
    QTest::newRow("compressed tarball")
        << QStringLiteral("sources.tar.gz") << QStringLiteral("archive");
}
...
void DownloadPolicyTest::readsTheKindFromTheTypeWhereTheNameCannotSay_data()
{
    ...
    QTest::newRow("Android package")
        << QStringLiteral("application/vnd.android.package-archive") << QStringLiteral("installer");
    QTest::newRow("Apple disk image")
        << QStringLiteral("application/x-apple-diskimage") << QStringLiteral("disk image");
    QTest::newRow("zip archive") << QStringLiteral("application/zip") << QStringLiteral("archive");
    QTest::newRow("type with a parameter")
        << QStringLiteral("Application/Zip; charset=binary") << QStringLiteral("archive");
}
...
void DownloadPolicyTest::namesNoKindForAnEverydayDocument_data()
{
    ...
    QTest::newRow("PDF") << QStringLiteral("invoice.pdf") << QStringLiteral("application/pdf");
    ...
    QTest::newRow("executable renamed")
        << QStringLiteral("setup.exe.txt") << QStringLiteral("text/plain");
}
