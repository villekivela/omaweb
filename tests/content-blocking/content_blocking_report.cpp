...
QJsonObject runFixtures(const QJsonObject &fixtures, const omaweb::QtContentBlocker &adapter,
    const omaweb::ContentBlocker &contentBlocker)
{
    const auto resourceType = [](const QString &name) {
        using Info = QWebEngineUrlRequestInfo;
        if (name == QStringLiteral("document"))
            return Info::ResourceTypeMainFrame;
        if (name == QStringLiteral("subdocument"))
            return Info::ResourceTypeSubFrame;
        if (name == QStringLiteral("stylesheet"))
            return Info::ResourceTypeStylesheet;
        if (name == QStringLiteral("script"))
            return Info::ResourceTypeScript;
        if (name == QStringLiteral("image"))
            return Info::ResourceTypeImage;
        if (name == QStringLiteral("font"))
            return Info::ResourceTypeFontResource;
        if (name == QStringLiteral("media"))
            return Info::ResourceTypeMedia;
        if (name == QStringLiteral("xmlhttprequest"))
    ...
    for (const auto &value : fixtures.value(QStringLiteral("scriptlet")).toArray()) {
        const auto fixture = value.toObject();
        const auto actual
            = contentBlocker.scriptletSource(QUrl(fixture.value(QStringLiteral("url")).toString()))
    ...
    for (const auto &value : fixtures.value(QStringLiteral("cosmetic")).toArray()) {
        const auto fixture = value.toObject();
        const auto actual
            = adapter.cosmeticStyleSheet(QUrl(fixture.value(QStringLiteral("url")).toString()))
                  .contains(fixture.value(QStringLiteral("contains")).toString());
    ...
    for (const auto &value : fixtures.value(QStringLiteral("cosmeticSurveyWanted")).toArray()) {
        const auto fixture = value.toObject();
        const auto actual
            = adapter.cosmeticSurveyWanted(QUrl(fixture.value(QStringLiteral("url")).toString()));
    ...
    for (const auto &value : fixtures.value(QStringLiteral("cosmeticSurvey")).toArray()) {
        ...
        const auto actual
            = adapter
                  .genericCosmeticStyleSheet(QUrl(fixture.value(QStringLiteral("url")).toString()),
...
int main(int argc, char *argv[])
{
    ...
    QEventLoop compiled;
    QObject::connect(
        &contentBlocker, &omaweb::ContentBlocker::rulesChanged, &compiled, &QEventLoop::quit);
    ...
    return output.commit() && result.value(QStringLiteral("status")) == QStringLiteral("pass") ? 0
                                                                                               : 1;
}
